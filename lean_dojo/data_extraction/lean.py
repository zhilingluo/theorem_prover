...
LEAN4_REPO = None# GITHUB.get_repo("leanprover/lean4")
"""The GitHub Repo for Lean 4 itself."""

LEAN4_NIGHTLY_REPO =None# GITHUB.get_repo("leanprover/lean4-nightly")
...
class dummyObject():
    def __init__(self,name):
        self.name=name
def url_to_local_repo(url: str, num_retries: int = 2) -> Repository:
    res = dummyObject(url)
...
#            raise ValueError(f"{self.url} is not a valid URL")
        ...
        object.__setattr__(self, "repo", url_to_local_repo(self.url))
        ...
        if (self.url, self.commit) in info_cache.lean_version:
            ...
            lean_version = 'leanprover/lean4:v4.7.0'#get_lean4_commit_from_config(config)
